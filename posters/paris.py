...
# DATA (2014) = https://www.zipparis.com/#!/tellmap/1720012440
...
geo_file = "geojson/France/Communes_MGP.json"

df = geopandas.read_file(geo_file)
# df_json = df.to_json()


df['density'] = df['POP_14']/(df['SHAPE_Area']/1000000)
df.to_file("paris_id.geojson", driver='GeoJSON')


with open('paris_id.geojson') as jsonfile:
    final = json.load(jsonfile)

for i in final['features']:
    i['id'] = i['properties']['ID_APUR']

if os.path.exists("paris_final.geojson"):
    os.remove("paris_final.geojson")
else:
    print('file doesn''t exist')

with open("paris_final.geojson", "w") as jsonFileFinal:
...
with open('paris_final.geojson') as json_file:
...
fig = go.Figure(go.Choroplethmapbox(
    geojson=paris,
    locations=df['ID_APUR'], # Spatial coordinates
    z = df['density'], # Data to be color-coded
    # locationmode = 'USA-states', # set of locations match entries in `locations`
    colorscale=[[0, '#ffc33e'], [0.5, '#ff0000'], [1, '#000']], zmin=0,zmax=40000,
    marker_opacity=0.65, marker_line_width=1.5,marker_line_color='#fff',text=df['L_CAB']
))
...
# original
fig.write_image("images/paris.png", engine="kaleido")
print("image saved")
