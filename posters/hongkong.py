...
# GEO (2020) = https://opendata.esrichina.hk/datasets/eea8ff2f12b145f7b33c4eef4f045513_0/data
# DATA (2019) = https://www.censtatd.gov.hk/hkstat/sub/sp150.jsp?productCode=D5320189
...
geo_file = "geojson/HongKong/hk.geojson"
...
with open("hongkong_id.geojson") as jsonfile:
    final = json.load(jsonfile)

for i in final['features']:
    i['id'] = i['properties']['CNAME']

if os.path.exists("hongkong_final.geojson"):
    os.remove("hongkong_final.geojson")
else:
    print('file doesn''t exist')

with open("hongkong_final.geojson", "w") as jsonFileFinal:
...
with open('hongkong_final.geojson') as json_file:
    hk = json.load(json_file)


fig = go.Figure(go.Choroplethmapbox(
    geojson=hk,
    locations=merged['CNAME'], # Spatial coordinates
    z = merged['density'], # Data to be color-coded
    # locationmode = 'USA-states', # set of locations match entries in `locations`
    colorscale=[[0, '#ffc33e'], [0.3, '#ff0000'], [1, '#000']], zmin=0,zmax=60000,
    marker_opacity=0.65, marker_line_width=1.5,marker_line_color='#fff',text=merged['ENAME']
))
...
# original
fig.write_image("images/hk.png", engine="kaleido")
print("image saved")
