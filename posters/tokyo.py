...
# GEO = https://github.com/dataofjapan/land/blob/master/tokyo.geojson
# DATA = https://www.citypopulation.de/php/japan-admin.php
...
geo_file = "geojson/Japan/tokyo.geojson"

df = geopandas.read_file(geo_file)
# df_json = df.to_json()
data = pd.read_csv('data/tokyo.csv', encoding = "utf-8-sig")#ISO-8859-1

# #005a64
# #00a0b0

# join the geodataframe with the csv dataframe
merged = df.merge(data, how='left', left_on="ward_ja", right_on="native")
...
merged['pop_density'].fillna(0,inplace=True)
merged['pop_density'].replace('None',0, inplace=True)
merged.to_file("tokyo.geojson", driver='GeoJSON')
print(merged)

with open('tokyo.geojson') as jsonfile:
    final = json.load(jsonfile)

for i in final['features']:
    i['id'] = i['properties']['ward_ja']

if os.path.exists("tokyo_final.geojson"):
    os.remove("tokyo_final.geojson")
else:
    print('file doesn''t exist')

with open("tokyo_final.geojson", "w") as jsonFileFinal:
...
with open('tokyo_final.geojson') as json_file:
    tokyo = json.load(json_file)
...
fig = go.Figure(go.Choroplethmapbox(
    geojson=tokyo,
    locations=data['native'], # Spatial coordinates
    z = data['pop_density'], # Data to be color-coded
    # locationmode = 'USA-states', # set of locations match entries in `locations`
    colorscale=[[0, '#ffc33e'], [0.5, '#ff0000'], [1, '#000']], zmin=0,zmax=24000,
    marker_opacity=0.65, marker_line_width=1.5,marker_line_color='#fff',text=data['name']
))
...
# original
fig.write_image("images/tokyo.png", engine="kaleido")
print("image saved")
