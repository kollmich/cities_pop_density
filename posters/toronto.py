...
# GEO (2010) = https://github.com/codigourbano/subprefeituras-sp/blob/master/data/subprefeituras-sp.geojson
# DATA (2010) = https://github.com/codigourbano/subprefeituras-sp/blob/master/data/subprefeituras-sp.geojson
...
geo_file = "geojson/Canada/toronto.geojson"

df = geopandas.read_file(geo_file)
df['density'] = df['totalpop'] / df['totalarea']
...
# print(merged)
df.to_file("toronto.geojson", driver='GeoJSON')

with open("toronto.geojson") as jsonfile:
    final = json.load(jsonfile)

for i in final['features']:
    i['id'] = i['properties']['id']

if os.path.exists("toronto_final.geojson"):
    os.remove("toronto_final.geojson")
else:
    print('file doesn''t exist')

with open("toronto_final.geojson", "w") as jsonFileFinal:
...
with open('toronto_final.geojson') as json_file:
    toronto = json.load(json_file)
...
fig = go.Figure(go.Choroplethmapbox(
    ...
    colorscale=[[0, '#ffc33e'], [0.15, '#ff0000'], [1, '#000']], zmin=0,zmax=40000,
    marker_opacity=0.65, marker_line_width=1.5,marker_line_color='#fff',text=df['hood'] 
))
...
# original
fig.write_image("images/toronto.png", engine="kaleido")
print("image saved")
